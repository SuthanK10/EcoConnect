  ...
  <div class="max-w-4xl w-full grid grid-cols-1 md:grid-cols-2 bg-white dark:bg-darkSurface rounded-[40px] shadow-2xl overflow-hidden border border-gray-100 dark:border-white/5 relative z-10">
    ...
    <div class="hidden md:flex flex-col justify-between p-12 bg-[#2c4931] relative overflow-hidden">
      ...
      <div class="relative z-10">
        ...
        <h2 class="text-4xl font-black text-white leading-tight mb-4">Welcome back to EcoConnect</h2>
        <p class="text-white/70 text-lg">Continue your journey in making Sri Lanka cleaner and greener.</p>
      </div>
      <div class="relative z-10">
    ...
    <div class="p-8 md:p-12 flex flex-col justify-center">
      <div class="mb-10 text-center md:text-left">
        <h1 class="text-3xl font-black text-[#121613] dark:text-white mb-2">Login</h1>
        <p class="text-[#677e6b] dark:text-gray-400 font-medium lowercase tracking-wide">Enter your credentials to access your dashboard</p>
      </div>

      <?php if (!empty($error)): ?>
        <div class="mb-8 p-4 rounded-2xl bg-red-50 dark:bg-red-900/20 border border-red-100 dark:border-red-900/30 flex items-start gap-3 animate-shake">
          <span class="text-red-500 text-lg">
            <i data-lucide="alert-triangle" class="w-5 h-5"></i>
          </span>
          <div class="flex-1">
            <p class="text-sm text-red-700 dark:text-red-400 font-bold"><?php echo h($error); ?></p>
            <?php if (isset($deactivated_user_id)): ?>
              <form action="index.php?route=submit_appeal" method="post" class="mt-3">
                <?php echo csrf_field(); ?>
                <input type="hidden" name="user_id" value="<?php echo (int)$deactivated_user_id; ?>">
                <button type="submit" class="text-xs font-black uppercase tracking-widest text-red-800 dark:text-red-300 underline underline-offset-4 hover:text-red-600 transition-colors">
      ...
      <form method="post" class="space-y-6">
        ...
        <div class="group">
          <div class="flex justify-between items-center mb-2">
            <label class="block text-xs font-black uppercase tracking-widest text-[#677e6b] dark:text-gray-500 group-focus-within:text-[#2c4931] transition-colors">Password</label>
            <a href="index.php?route=forgot_password" class="text-xs font-bold text-[#2c4931] dark:text-[#4ade80] hover:underline">Forgot?</a>
          </div>
          <div class="relative group">
            ...
            <button 
              type="button"
              onclick="togglePassword('loginPassword', 'eyeIcon')"
              class="absolute inset-y-0 right-0 pr-4 flex items-center text-gray-400 hover:text-[#2c4931] transition-colors"
            >
              <span id="eyeIcon"><i data-lucide="eye" class="w-4 h-4"></i></span>
            </button>
          </div>
        </div>
        ...
        >
          Login to Account <i data-lucide="log-in" class="w-4 h-4"></i>
        </button>
        ...
        <div class="pt-6 text-center border-t border-gray-50 dark:border-white/5">
          <p class="text-sm font-medium text-[#677e6b] dark:text-gray-400">
            ...
            <a href="index.php?route=register" class="text-[#2c4931] dark:text-[#4ade80] font-black hover:underline ml-1">Create Account</a>
          </p>
        </div>
      </form>
    </div>
